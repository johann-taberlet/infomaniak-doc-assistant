/**
 * TTS Helper - TypeScript port of supertonic-web helper.js
...
export type TTSLang = (typeof AVAILABLE_LANGS)[number]

export function isValidLang(lang: string): lang is TTSLang {
  return AVAILABLE_LANGS.includes(lang as TTSLang)
}
...
class UnicodeProcessor {
  ...
  private getTextMask(textIdsLengths: number[]) {
    const maxLen = Math.max(...textIdsLengths)
 ...
 */
export async function loadTextToSpeechParallel(
  onnxDir: string,
  ...
  const models: Record<string, ort.InferenceSession> = {}
...
export async function checkWebGPUSupport(): Promise<boolean> {
  try {
    ...
    const isSafari =
      /^((?!chrome|crios|android|edg|firefox).)*safari/i.test(navigator.userAgent)
