// TTS Helper - Adapted from supertonic-web for browser ES module use
// Requires: ONNX Runtime Web loaded globally or via import map
...
export class TextToSpeech {
    ...
    async _infer(textList, langList, style, totalStep, speed = 1.05, progressCallback = null) {
        ...
        let { xt, latentMask } = this.sampleNoisyLatent(
            duration, this.sampleRate, this.cfgs.ae.base_chunk_size,
            this.cfgs.ttl.chunk_compress_factor, this.cfgs.ttl.latent_dim
        );
 ...
 */
export async function loadVoiceStyle(voiceStylePaths, ort, verbose = false) {
 ...
 * Load ONNX model
 */
export async function loadOnnx(onnxPath, options, ort) {
    return await ort.InferenceSession.create(onnxPath, options);
}

/**
 * Load all TTS components (sequential)
 */
export async function loadTextToSpeech(onnxDir, ort, sessionOptions = {}, progressCallback = null) {
    ...
    for (let i = 0; i < modelPaths.length; i++) {
        ...
        const session = await loadOnnx(modelPaths[i].path, sessionOptions, ort);
    ...
    const textToSpeech = new TextToSpeech(cfgs, textProcessor, dpOrt, textEncOrt, vectorEstOrt, vocoderOrt, ort);

    return { textToSpeech, cfgs };
}

/**
 * Load all TTS components (parallel - faster)
 */
export async function loadTextToSpeechParallel(onnxDir, ort, sessionOptions = {}, progressCallback = null) {
    console.log('Loading TTS models in parallel...');

    const cfgs = await loadCfgs(onnxDir);

    const modelPaths = [
        { name: 'Duration Predictor', path: `${onnxDir}/duration_predictor.onnx`, key: 'dp' },
        { name: 'Text Encoder', path: `${onnxDir}/text_encoder.onnx`, key: 'textEnc' },
        { name: 'Vector Estimator', path: `${onnxDir}/vector_estimator.onnx`, key: 'vectorEst' },
        { name: 'Vocoder', path: `${onnxDir}/vocoder.onnx`, key: 'vocoder' }
    ];

    let loadedCount = 0;
    const loadPromises = modelPaths.map(async (model) => {
        const session = await ort.InferenceSession.create(model.path, sessionOptions);
        loadedCount++;
        if (progressCallback) {
            progressCallback(model.name, loadedCount, modelPaths.length);
        }
    ...
    const textToSpeech = new TextToSpeech(
        cfgs, textProcessor,
        models.dp, models.textEnc, models.vectorEst, models.vocoder,
        ort
    );
