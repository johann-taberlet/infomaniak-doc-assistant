# Connect external applications to k Chat

Source: https://www.infomaniak.com/en/support/faq/2001/connect-external-applications-to-kchat

---

This guide allows you to manage external applications with [k Chat](https://infomaniak.com/gtl/kchat) using webhooks.

### Preamble

- A webhook is a method that allows an application to be immediately informed when a particular event occurs in another application, rather than constantly asking this application if something new has happened ("polling"). Outgoing webhook: k Chat communicates information to other apps when an event occurs in k Chat. Incoming webhook: k Chat receives information from other apps to trigger actions in k Chat.
...
## Access the k Chat webhooks interface
...
1. [Click here](https://kchat.infomaniak.com/)to access the **k Chat** Web app (online service [ksuite.infomaniak.com/kchat](https://ksuite.infomaniak.com/kchat)) or open the **k Chat** desktop app (desktop application on macOS / Windows / Linux).
2. Click on the **New** icon  next to your k Chat organization's name.
...
- [Reminder of Infomaniak calendar event on k Chat](https://www.infomaniak.com/en/support/faq/2244)
...
On the application that needs to post on k Chat:
...
> **Note:** The BOT indication is added next to the username on k Chat for security reasons.
